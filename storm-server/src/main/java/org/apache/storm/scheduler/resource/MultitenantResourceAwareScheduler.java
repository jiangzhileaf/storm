...
public class MultitenantResourceAwareScheduler implements IScheduler {
    ...
    private boolean validWorkerBandwidth(Integer workerBandwith) {
        Set<Integer> acceptedValue = new HashSet<>((List<Integer>) conf.get(Config.TOPOLOGY_WORKER_ACCEPT_BANDWIDTH_MBPS));
        return acceptedValue.contains(workerBandwith);
    }
    ...
    private void scheduleTopology(TopologyDetails td, Cluster cluster, final User topologySubmitter,
        ...
        Integer workerBandwith = ObjectReader.getDouble(td.getConf().get(Config.TOPOLOGY_WORKER_MAX_BANDWIDTH_MBPS)).intValue();

        if (!validWorkerBandwidth(workerBandwith)) {
            markFailedTopology(topologySubmitter, cluster, td,
                    "Unsuccessful in scheduling - worker bandwidth [" + workerBandwith + "] is not valid !");
            return;
        }
     ...
     *
     * @return a map that contains resource guarantees of every user of the following format {userid->{resourceType->amountGuaranteed}}
     */
